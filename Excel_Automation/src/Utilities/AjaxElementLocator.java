...
public class AjaxElementLocator extends DefaultElementLocator {
   ...
   * 
   * @param driver
   *            The WebDriver to use when locating elements
   * @param field
  ...
  public WebElement findElement() {
    ...
    timeOutInSeconds = Integer.parseInt(properties.getWaitTime());
    ...
    try {
      ...
      throw new NoSuchElementException(String.format("Timed out after %d seconds. %s", timeOutInSeconds, e.getMessage()), e.getCause());
    }
  }
  ...
  /**
   * By default, elements are considered "found" if they are in the DOM. Override this method in order to change whether or not you consider the elemet loaded. For example, perhaps you need the
